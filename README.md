...
If this option and the previous (`--IPFS`) option are both present, then FidoRSS uses IPFS URLs (pointing to the default https://ipfs.io/ gateway) instead of FGHI URLs as the addresses of RSS items (and also of FGHI URLs encountered inside Fidonet messages). These IPFS URLs are addresses of a small IPFS-hosted page that receives real FGHI URLs in its query string and generates a hyperlink by JavaScript. (Such page is automatically generated by Fido2RSS and published in IPFS on the fly.) This option is designed as a workaround for RSS consumers that do not expect FGHI URLs to appear in RSS. (For example, [LiveJournal](http://www.livejournal.com/) simply replaces `area:` with `http:` in FGHI URLs, which is wrong.)
...
* `options.IPFSURL` — if this option is a truthy value and the previous option (`options.IPFS`) is not `undefined`, then FidoRSS uses IPFS URLs (pointing to the default https://ipfs.io/ gateway) instead of FGHI URLs as the addresses of RSS items (and also of FGHI URLs encountered inside Fidonet messages).
   * Such IPFS URLs are addresses of a small IPFS-hosted page that receives real FGHI URLs in its query string and generates a hyperlink by JavaScript. (Such page is automatically generated by Fido2RSS and published in IPFS on the fly.)
